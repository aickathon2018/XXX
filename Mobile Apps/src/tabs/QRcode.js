...
import {getProductDetails} from '../components/RealTimeDatabase'
...
class QRCode extends React.Component {
  ...
  constructor(props){
    ...
    //this._onLearnMore("001")
    })
  }

  _onLearnMore = async(userid) =>{
    const user = await getProductDetails(userid)
    console.log(userid)
  ...
  render() {
    const scan = this.state.ready?(
        ...
        bottomContent={
            <Text style={styles.centerText}>QR Code can be found on Profile</Text>
        }
      />
    ):(<Text> </Text>)
