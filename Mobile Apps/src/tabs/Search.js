...
import {getProductDetails, searchProduct} from '../components/RealTimeDatabase'
...
class Contact extends React.Component {
  ...
  _willBlurSubscription;
    ...
    _click = async(name) => {
      ...
      var id = await searchProduct(name)
    ...
    _onSearch = async(id) =>{
      ...
      {
        users.push(await getProductDetails(id[i]))
      }
          ...
          <List containerStyle={{borderTopWidth:0, borderBottomWidth:0}}>
            {this.state.users.map((product) => (
                <ListItem
                  ...
                  roundAvatar
                  avatar={{ uri: product.imageURL }}s
                  title={`${product.name.toUpperCase()}`}
                  subtitle={product.price}
                  onPress={() => this.onLearnMore(product)}
                />
              ))}
          </List>
        </ScrollView>
